...
func (c Constant) IsDimless() bool {
	return len(c.Dimensions) == 0
}

func (c Constant) IsBase() bool {
	return len(c.Dimensions) == 1 && c.Dimensions[0].Power == 1
}
...
func (c Constant) Type() string {
	...
	if c.IsBase() {
		return "unit." + strings.TrimSuffix(c.Dimensions[0].Name, "Dim")
	}
...
// The dimension of {{.Name}} is {{.Units}}.{{end}}
...
// {{.Comment}}
// {{$n := len .Dimensions}}The dimension{{if gt $n 1}}s{{end}} of {{.Name}} {{if eq $n 1}}is{{else}}are{{end}} {{.Units}}.
const {{.Name}} = {{.Type}}({{.Value}})
